...
import { useDeletePermohonanCuti } from "../controller";
import { convertFromSnakeCase } from "@/utils/helpers";

export function ButtonDelete({ rowData }: { rowData?: PermohonanCuti }) {
	const { disabled, setIsShow, setSelected, handleDelete, isShow, selected } =
		useDeletePermohonanCuti();
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Hapus Permohonan Cuti</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menghapusPermohonan Cuti ini?
						</DialogDescription>
					</DialogHeader>
					...
					<div className="flex flex-col gap-4 p-3 bg-[#F6FFF5] border border-[#9EDAA0] rounded-md">
						...
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Jenis Cuti</p>
							<p className="flex-1">{selected?.jenis_cuti || "-"}</p>
						</div>
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Keperluan</p>
							<p className="flex-1">{selected?.alasan_cuti || "-"}</p>
						</div>
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Periode Cuti</p>
							<p className="flex-1">
								{selected?.mulai
