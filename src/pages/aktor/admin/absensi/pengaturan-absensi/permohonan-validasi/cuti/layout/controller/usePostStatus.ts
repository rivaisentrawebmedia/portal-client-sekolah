...
export function usePostStatusPermohonanCuti() {
	...
	const mutation = useMutation({
		mutationFn: (payload: PayloadStatusPengajuan) =>
			AxiosClient.post("/presensi/pengajuan-cuti/set-status", payload),
		...
		onSuccess: async (_, __, context) => {
			await queryClient.invalidateQueries({ queryKey: ["pengajuan-cuti"] });
