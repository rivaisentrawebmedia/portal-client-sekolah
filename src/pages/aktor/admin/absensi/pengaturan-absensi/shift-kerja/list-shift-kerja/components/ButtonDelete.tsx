...
import { useDeleteShiftKerja } from "../controller";
import {
	Dialog,
	DialogContent,
	DialogDescription,
...
export function ButtonDelete({ rowData }: { rowData?: ShiftKerja }) {
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Hapus Shift Kerja</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menghapus Shift Kerja ini?
						</DialogDescription>
					</DialogHeader>
					...
					<div className="flex flex-col gap-4 p-3 bg-[#F6FFF5] border border-[#9EDAA0] rounded-md">
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Nama Shift</p>
