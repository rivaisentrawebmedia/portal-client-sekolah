...
import { useUpdateKegiatanHarian } from "../list-kegiatan-harian/controller";
import { usePathname } from "@/utils/usePathname";
import {
	FormUploadDokumen,
	FormUploadGambar,
	FormData,
} from "../buat-kegiatan-harian/components";

export default function EditKegiatanHarianPage() {
	const navigate = useNavigate();
	const { fivethPathname } = usePathname();
	const [params] = useSearchParams();

	const { form, disabled, isShow, onSubmit, setIsShow } =
		useUpdateKegiatanHarian();
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Edit Kegiatan Harian</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin mengubah Kegiatan Harian berikut ini?
						</DialogDescription>
					</DialogHeader>
