...
export function FormUploadDokumen({
	...
	const handleUpload = async (
		...
		await onSubmitUploadFile({
			form,
			fields: `lampiran_dokumen.${index}.id`,
			file,
			label: `lampiran_dokumen.${index}.label`,
		});
	...
	// upload untuk lampiran_dokumen BARU
	const handleUploadNew = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		setTimeout(async () => {
			await onSubmitUploadFile({
				form,
				fields: `lampiran_dokumen.${newIndex}.id`,
				file,
				label: `lampiran_dokumen.${newIndex}.label`,
			});
		}, 0);
	...
	return (
		<div className="flex flex-col col-span-1 md:col-span-2 gap-3">
			<p className="text-neutral font-normal text-sm">Lampiran Dokumen</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				{/* Tombol Tambah lampiran_dokumen (UI TETAP) */}
				<button
					type="button"
					disabled={loading}
