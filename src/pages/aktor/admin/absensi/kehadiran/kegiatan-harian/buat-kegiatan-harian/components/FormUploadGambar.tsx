...
export function FormUploadGambar({
	...
	const handleUpload = async (
		...
		await onSubmitUploadFile({
			form,
			fields: `lampiran_gambar.${index}.id`,
			file,
			label: `lampiran_gambar.${index}.label`,
		});
	...
	// upload untuk lampiran_gambar BARU
	const handleUploadNew = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		setTimeout(async () => {
			await onSubmitUploadFile({
				form,
				fields: `lampiran_gambar.${newIndex}.id`,
				file,
				label: `lampiran_gambar.${newIndex}.label`,
			});
		}, 0);
	...
	return (
		<div className="flex flex-col col-span-1 md:col-span-2 gap-3">
			<p className="text-neutral font-normal text-sm">Lampiran Gambar</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				{fields.map((field, index) => (
					...
					>
						<label className={clsx("cursor-pointer")}>
							{form.watch(`lampiran_gambar.${index}.id`) !== "" ? (
								<div className="relative flex">
									<img
										src={form.watch(`lampiran_gambar.${index}.id`) || ""}
										alt={`lampiran_gambar.${index}.label`}
				...
				{/* Tombol Tambah lampiran_gambar (UI TETAP) */}
				<button
					type="button"
					disabled={loading}
