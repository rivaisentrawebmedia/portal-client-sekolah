...
import { useDeleteBagianSurat } from "../controller";
import {
	Dialog,
	DialogContent,
	DialogDescription,
...
export function ButtonDelete({ rowData }: { rowData?: BagianSurat }) {
	const { disabled, setIsShow, setSelected, handleDelete, isShow, selected } =
		useDeleteBagianSurat();
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Hapus Bagian Surat</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menghapus Bagian Surat ini?
						</DialogDescription>
					</DialogHeader>
					...
					<div className="flex flex-col gap-4 p-3 bg-[#eaf1f9]/50 border border-[#161646]/50 rounded-md">
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Bagian Surat</p>
