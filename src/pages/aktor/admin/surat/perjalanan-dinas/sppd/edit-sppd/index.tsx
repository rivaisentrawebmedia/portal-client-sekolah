...
import { useUpdateSPPD } from "../list-sppd/controller";
...
import { FormPilihPegawai } from "./FormPilihPegawai";

export default function EditSPPDPage() {
	const { fivethPathname } = usePathname();
	const navigate = useNavigate();
	const [params] = useSearchParams();
	const { disabled, form, isShow, onSubmit, setIsShow, loading } =
		useUpdateSPPD();
	...
	if (loading) {
		return <LoadingSpinner className="min-w-[80vh]" />;
	}
		...
		<>
			<Form {...form}>
				...
				>
					...
					<div className="flex flex-col gap-4">
						...
						<FormPilihPegawai disabled={disabled} form={form} />
					</div>
				</form>
			</Form>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Edit SPPD</DialogTitle>
						<DialogDescription>
