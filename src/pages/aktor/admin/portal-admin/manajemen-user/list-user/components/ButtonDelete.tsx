...
import { useDeleteManajemenUser } from "../controller";

export function ButtonDelete({ rowData }: { rowData?: ManajemenUser }) {
	const { disabled, setIsShow, setSelected, handleDelete, isShow, selected } =
		useDeleteManajemenUser();
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					...
					<div className="flex flex-col gap-4 p-3 bg-[#F6FFF5] rounded-md border border-primary/50">
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Nama Lengkap</p>
							<p className="flex-1">{selected?.nama || "-"}</p>
						</div>
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Jabatan</p>
							<p className="flex-1">{selected?.jabatan || "-"}</p>
						</div>
					</div>
