...
export default function EditUserPage() {
	...
	const organisasi_id = params.get("user-id");
	...
	return (
		<>
			<div className="flex flex-col gap-4 w-full">
				<ArrowBack
					link={`/admin/manajemen-user?${params?.toString()}`}
					title="Edit User"
				/>
				<div className="flex flex-col gap-4 w-full md:flex-row bg-[#F6FFF5] rounded-md p-4">
					<div
						onClick={() => {
							navigate(
								`/admin/manajemen-user/edit-user?user-id=${organisasi_id}`,
							);
						}}
						className={clsx(
					...
					<div
						onClick={() => {
							navigate(
								`/admin/manajemen-user/edit-user/kontrol-akses?user-id=${organisasi_id}`,
							);
						}}
						className={clsx(
							"w-full gap-2 border duration-300 transition-colors rounded-md cursor-pointer bg-white p-2 flex items-center justify-center",
