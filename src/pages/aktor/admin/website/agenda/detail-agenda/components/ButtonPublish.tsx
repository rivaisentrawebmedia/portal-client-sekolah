...
import type { Agenda } from "../../list-agenda/model";
import { usePostStatusStatusAgenda } from "../../list-agenda/controller";

export function ButtonPublishAgenda({ rowData }: { rowData?: Agenda }) {
	const { disabled, setIsShow, isShow, onSubmit, setCheckedPool } =
		usePostStatusStatusAgenda();
		...
		<>
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Publish Agenda</DialogTitle>
						<DialogDescription>
							Anda akan mempublish Agenda dan Agenda akan dapat dibaca oleh
					...
					<div className="flex flex-col gap-4 p-3 bg-[#F5F9FF]">
						<div className="flex flex-col font-sans text-sm rounded-md md:flex-row md:items-center md:gap-2">
							<p className="w-full md:w-1/3 text-[#888]">Judul Agenda</p>
