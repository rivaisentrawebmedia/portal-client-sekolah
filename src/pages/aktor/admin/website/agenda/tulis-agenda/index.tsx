...
import {
	useGetKategoriAgenda,
	useGetTagAgenda,
	usePostAgenda,
} from "../list-agenda/controller";
...
export default function TulisAgendaPage() {
	const navigate = useNavigate();
	const [params] = useSearchParams();
	const { disabled, form, isShow, onSubmit, setIsShow } = usePostAgenda();
	...
	const { data: kategoriAgendaOptions, loading: loadingAgenda } =
		useGetKategoriAgenda();

	const { data: tagAgendaOptions, loading: loadingTag } = useGetTagAgenda();
		...
		<>
			<Form {...form}>
				...
				>
					<div className="flex flex-col gap-2 md:flex-row md:justify-between md:gap-4">
						<ArrowBack
							link={`/admin/website/agenda?${params?.toString()}`}
							title="Tulis Agenda"
							background="#F5F9FF"
							border="#F5F9FF"
							text="#276CCD"
						...
						<div className="flex gap-2 flex-col md:flex-row">
							<Button
								...
								onClick={() => {
									navigate(`/admin/website/agenda/tulis-agenda`);
								}}
								variant={"outline"}
							>
								<X />
					...
					<div className="flex flex-col gap-4">
						...
						<SelectCommon
							form={form}
							name="kategori_agenda_id"
							disabled={disabled || loadingAgenda}
							...
							options={
								kategoriAgendaOptions?.map((item) => {
									return {
										label: item?.nama,
										value: item?.id,
									};
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Tulis Agenda</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menyimpan Agenda ini?
						</DialogDescription>
					</DialogHeader>
