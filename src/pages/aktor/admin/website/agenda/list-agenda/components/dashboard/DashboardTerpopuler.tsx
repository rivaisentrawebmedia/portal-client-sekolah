...
export function DashboardAgendaTerpopuler() {
	const navigate = useNavigate();
	const [searchParams] = useSearchParams();

	const page = searchParams.get("page-agenda-terpopuler");
	const limit = searchParams.get("limit-agenda-terpopuler");
	const search = searchParams.get("search-agenda-terpopuler");
	...
	const { data, loading } = useGetAgenda(paramsDefault);

	return (
		<div className="flex flex-col gap-2 bg-[#F5F9FF] rounded-md p-3 border border-[#70A6F2]">
			<p className="text-[#276CCD] font-medium">Agenda Terpopuler</p>
			...
			{!loading && data?.length === 0 && (
				<Empty className="bg-white border border-dashed">
					<EmptyHeader>
						...
						<EmptyTitle>Belum Ada Agenda</EmptyTitle>
						<EmptyDescription>
							Belum ada Agenda yang dipublikasikan saat ini.
						</EmptyDescription>
					</EmptyHeader>

					<EmptyContent>
						<p>Agenda terbaru akan muncul di sini setelah dipublikasikan.</p>
					</EmptyContent>
				</Empty>
			)}
			...
			{!loading &&
				data?.slice(0, 5)?.map((item, idx) => (
					<div
						...
						onClick={() => {
							navigate(
								`/admin/website/agenda/${item?.id}/detail?${searchParams.toString()}`,
							);
						}}
					>
