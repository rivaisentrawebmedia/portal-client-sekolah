...
import {
	useGetKategoriPrestasi,
	useGetTagPrestasi,
	usePostPrestasi,
} from "../list-prestasi/controller";
...
export default function TulisPrestasiPage() {
	const navigate = useNavigate();
	const [params] = useSearchParams();
	const { disabled, form, isShow, onSubmit, setIsShow } = usePostPrestasi();
	...
	const { data: kategoriPrestasiOptions, loading: loadingPrestasi } =
		useGetKategoriPrestasi();

	const { data: tagPrestasiOptions, loading: loadingTag } = useGetTagPrestasi();
		...
		<>
			<Form {...form}>
				...
				>
					<div className="flex flex-col gap-2 md:flex-row md:justify-between md:gap-4">
						<ArrowBack
							link={`/admin/website/prestasi?${params?.toString()}`}
							title="Tulis Prestasi"
							background="#F5F9FF"
							border="#F5F9FF"
							text="#276CCD"
						/>
						...
						<div className="flex gap-2 flex-col md:flex-row">
							<Button
								...
								onClick={() => {
									navigate(`/admin/website/prestasi/tulis-prestasi`);
								}}
								variant={"outline"}
							>
								<X />
					...
					<div className="flex flex-col gap-4">
						...
						<SelectCommon
							...
							disabled={disabled || loadingPrestasi}
							label="Kategori Prestasi*"
							placeholder="Pilih kategori Prestasi"
							className="flex flex-col gap-2 md:flex-row"
							labelClassName="w-full md:w-1/4"
							selectClassName="flex-1"
							isLoading={loadingPrestasi}
							options={
								kategoriPrestasiOptions?.map((item) => {
									return {
										label: item?.nama,
										value: item?.id,
									};
						...
						<SelectCommon
							...
							label="Tag Prestasi*"
							placeholder="Pilih tag Prestasi"
							className="flex flex-col gap-2 md:flex-row"
							labelClassName="w-full md:w-1/4"
							selectClassName="flex-1"
						...
						<TextEditorCommon
							...
							disabled={disabled}
							label="Isi*"
							placeholder="Masukkan isi Prestasi"
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Tulis Prestasi</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menyimpan Prestasi ini?
						</DialogDescription>
					</DialogHeader>
