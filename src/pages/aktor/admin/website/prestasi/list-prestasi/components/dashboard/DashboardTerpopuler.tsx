...
export function DashboardPrestasiTerpopuler() {
	const navigate = useNavigate();
	const [searchParams] = useSearchParams();

	const page = searchParams.get("page-prestasi-terpopuler");
	const limit = searchParams.get("limit-prestasi-terpopuler");
	const search = searchParams.get("search-prestasi-terpopuler");
	...
	const { data, loading } = useGetPrestasi(paramsDefault);

	return (
		<div className="flex flex-col gap-2 bg-[#F5F9FF] rounded-md p-3 border border-[#70A6F2]">
			<p className="text-[#276CCD] font-medium">Prestasi Terpopuler</p>
			...
			{!loading && data?.length === 0 && (
				<Empty className="bg-white border border-dashed">
					<EmptyHeader>
						...
						<EmptyTitle>Belum Ada Prestasi</EmptyTitle>
						<EmptyDescription>
							Belum ada Prestasi yang dipublikasikan saat ini.
						</EmptyDescription>
					</EmptyHeader>

					<EmptyContent>
						<p>Prestasi terbaru akan muncul di sini setelah dipublikasikan.</p>
					</EmptyContent>
				</Empty>
			)}
			...
			{!loading &&
				data?.slice(0, 5)?.map((item, idx) => (
					<div
						...
						onClick={() => {
							navigate(
								`/admin/website/prestasi/${item?.id}/detail?${searchParams.toString()}`,
							);
						}}
					>
