...
import { PrestasiSchema, updatePrestasi } from "../model";
import { useNavigate } from "react-router-dom";
import { useGetPrestasiByID } from "./useGetByID";
import dayjs from "dayjs";

export function useUpdatePrestasi() {
	const navigate = useNavigate();
	const { data: selected } = useGetPrestasiByID();
	...
	const mutation = useMutation({
		mutationFn: updatePrestasi,

		onMutate: () => {
			return toast.loading("Memperbarui data prestasi...");
		},
		...
		onSuccess: async (res, _variables, toastId) => {
			...
			toast.update(toastId, {
				render: res?.message || "Berhasil memperbarui prestasi",
				type: "success",
				isLoading: false,
				autoClose: 3000,
			});
		...
		if (selected) {
			form.reset({
				...
				tanggal: selected.tanggal
					...
					: "",
				kategori_prestasi_id: selected.kategori_prestasi_id,
				isi: selected.isi,
