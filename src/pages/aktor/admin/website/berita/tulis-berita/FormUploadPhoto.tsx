...
import type { BeritaFormValues } from "../list-berita/model";
				...
				<div className={clsx("whitespace-nowrap flex flex-col gap-2")}>
					...
					>
						{form.watch(name) ? (
							<div className="flex gap-2">
								{form.watch(name) ? (
									<img
										src={form.watch(name) || ""}
										alt={label}
										className="w-32 h-32 rounded-md bg-[#F5F5FF]"
									/>
								) : (
									<div className="w-32 h-32 rounded-md bg-[#276CCD] text-white flex items-center justify-center">
										{getInitials("Joko Wiseso")}
									</div>
								)}
								<div className="flex items-center justify-center">
									<div className="flex items-center gap-2 py-1.5 px-4 text-xs w-fit border border-[#162259] rounded-full">
										<FaUpload />
										Ganti File
									</div>
								</div>
							...
							<div className="flex gap-2">
								<div className="flex flex-col w-32 h-32 bg-[#F5F5FF] border border-[#DFDFDF] gap-2 items-center justify-center">
									<JokoAddPhoto />
								</div>
