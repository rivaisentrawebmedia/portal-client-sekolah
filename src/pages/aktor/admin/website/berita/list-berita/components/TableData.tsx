...
import type { Berita } from "../model";
import { SlideGambar } from "./SlideGambar";
import { ButtonPublishBerita } from "./ButtonPublishBerita";
import { ButtonDetail } from "./ButtonDetail";
import { ButtonEdit } from "./ButtonEdit";
import { ButtonDelete } from "./ButtonDelete";
import { ButtonKembaliKeDraft } from "./ButtonKembaliKeDraft";

dayjs.extend(relativeTime);
dayjs.locale("id");

interface TableBeritaProps {
	data: Berita[];
	page: number;
	limit: number;
	search: string;
...
export function TableBeritaDraft({
	data,
	page,
	limit,
	search,
	...
	return (
		<BaseTable
			...
			search={search}
			background="#F5F9FF"
			emptyTitle="Belum ada data Berita"
			emptyDescription="Silakan tambahkan data Berita terlebih dahulu"
			columns={[
				{
					header: "#",
				...
				},
				{
					header: "Kategori",
					className: "font-light text-[#276CCD]",
					render: (item) => <p>{item?.kategori_berita || "-"}</p>,
				},
				{
					...
					render: (item) => {
						return (
							<div className="flex flex-col gap-2">
								...
								<ButtonPublishBerita rowData={item} />
							</div>
						);
					},
				},
...
export function TableBeritaPublish({
	data,
	page,
	limit,
	search,
