...
export default function BeritaLayout() {
	...
	const listDashboard = ["Dashboard", "Published", "Draft"];

	return (
		<>
			<div className="flex flex-col gap-4">
				<div className="flex items-center justify-between gap-4">
					<p className="text-2xl font-medium">Berita</p>
					<Button
						type="button"
						onClick={() => {
							navigate("tulis-berita");
						}}
						variant={"outline"}
							...
							<div
								onClick={() => {
									if (item === listDashboard?.[0]) {
										navigate(`/admin/website/berita`);
									} else {
										navigate(`/admin/website/berita/${convertToSlug(item)}`);
									}
								}}
								key={idx}
								className={clsx(
