...
import {
	useGetKategoriMading,
	useGetTagMading,
	useUpdateMading,
} from "../list-mading/controller";
...
import { FormUploadGambar } from "../tulis-mading/FormUploadGambar";
import {
	InputCommon,
	SelectCommon,
	TextEditorCommon,
...
export default function EditMadingPage() {
	...
	const { disabled, form, isShow, onSubmit, setIsShow } = useUpdateMading();
	...
	const { data: kategoriMadingOptions, loading: loadingMading } =
		useGetKategoriMading();

	const { data: tagMadingOptions, loading: loadingTag } = useGetTagMading();
		...
		<>
			<Form {...form}>
				...
				>
					<div className="flex flex-col gap-2 md:flex-row md:justify-between md:gap-4">
						...
						<div className="flex gap-2 md:flex-row flex-col">
							<Button
								...
								onClick={() => {
									navigate(`/modules/modul-website/mading`);
								}}
								variant={"outline"}
							>
								<X />
					...
					<div className="flex flex-col gap-4">
						...
						<SelectCommon
							form={form}
							name="kategori_mading_id"
							disabled={disabled || loadingMading}
							label="Kategori Mading*"
							placeholder="Pilih kategori mading"
							className="flex flex-col gap-2 md:flex-row"
							labelClassName="w-full md:w-1/4"
							selectClassName="flex-1"
							isLoading={loadingMading}
							options={
								kategoriMadingOptions?.map((item) => {
									return {
										label: item?.nama,
										value: item?.id,
									};
						...
						<TextEditorCommon
							...
							disabled={disabled}
							label="Isi*"
							placeholder="Masukkan isi mading"
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Edit Mading</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menyimpan perubahan pada mading ini?
						</DialogDescription>
					</DialogHeader>
