...
export function DashboardPengumumanTerpopuler() {
	const navigate = useNavigate();
	const [searchParams] = useSearchParams();

	const page = searchParams.get("page-pengumuman-terpopuler");
	const limit = searchParams.get("limit-pengumuman-terpopuler");
	const search = searchParams.get("search-pengumuman-terpopuler");
	...
	const { data, loading } = useGetPengumuman(paramsDefault);

	return (
		<div className="flex flex-col gap-2 bg-[#F5F9FF] rounded-md p-3 border border-[#70A6F2]">
			<p className="text-[#276CCD] font-medium">Pengumuman Terpopuler</p>
			...
			{!loading &&
				data?.slice(0, 5)?.map((item, idx) => (
					<div
						...
						onClick={() => {
							navigate(
								`/admin/website/pengumuman/${item?.id}/detail?${searchParams.toString()}`,
							);
						}}
					>
