...
import { usePostHasilSekolah } from "./controller";
import { TextEditorCommon } from "@/components/common/basic-input";
import { FormItems, FormUploadPhoto } from "./components";
import { useNavigate } from "react-router-dom";

export function EditHasilPage() {
	const navigate = useNavigate();
	const { disabled, form, isShow, onSubmit, setIsShow } = usePostHasilSekolah();
		...
		<>
			<Form {...form}>
				...
				>
					<div className="flex md:justify-between gap-4">
						<p className="text-lg text-[#245192] font-medium">Edit Hasil</p>
					...
					<>
						...
						/>
						<FormItems form={form} isLoading={disabled} label="Hasil" />
					</>
				</form>
			</Form>
			...
			<Dialog open={isShowBatal} onOpenChange={setIsShowBatal}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Batal Ubah Hasil Sekolah</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin membatalkan perubahan Hasil sekolah ini?
			...
			<Dialog open={isShow} onOpenChange={setIsShow}>
				<DialogContent className="w-[95vw] max-w-md rounded-lg p-6 max-h-[90vh]">
					<DialogHeader>
						<DialogTitle>Ubah Hasil Sekolah</DialogTitle>
						<DialogDescription>
							Apakah anda yakin ingin menyimpan perubahan Hasil sekolah ini?
						</DialogDescription>
					</DialogHeader>
