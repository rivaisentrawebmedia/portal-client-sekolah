...
interface TablePermohonanIzinProps {
	data: PermohonanIzin[];
	page: number;
	limit: number;
	search: string;
...
export function TablePermohonanIzin({
	data,
	page,
	limit,
	search,
	...
	checked,
	setChecked,
}: TablePermohonanIzinProps) {
	return (
		<BaseTable
			data={data}
			loading={loading}
			search={search}
			emptyTitle="Belum ada data permohonan Izin"
			emptyDescription="Silakan tambahkan data permohonan izin terlebih dahulu"
			checkedPool={{
				getId: (row) => row.id,
				value: checked,
				onChange: setChecked,
			...
			columns={[
				...
				},
				{
					header: "Jenis Izin",
					className: "font-light text-[#1E5916]",
					render: (item) => <p>{item?.jenis_izin || "-"}</p>,
				},
				{
					header: "Keperluan",
					className: "font-light text-[#1E5916]",
					render: (item) => <p>{item?.alasan_izin || "-"}</p>,
				},
				{
					header: "Periode Izin",
